...
use App\ApiResource\Common\LinksHandlerTrait;
...
final class PersistProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): mixed
    {
        ...
        if ($operation instanceof HttpOperation && 'PUT' === $operation->getMethod() && ($operation->getExtraProperties()['standard_put'] ?? false)) {
            \assert(method_exists($newManager, 'getReference'));
            $newData = $data;
            ...
            if ($previousData = $context['previous_data']) {
                $classMetadata = $newManager->getClassMetadata($class);
                $identifiers = $classMetadata->getIdentifierValues($previousData);
                $newData = 1 === \count($identifiers) ? $newManager->getReference($class, current($identifiers)) : clone $previousData;
        ...
        if (!$newManager->contains($data) || $this->isDeferredExplicit($newManager, $data)) {
            $newManager->persist($data);
        }

        $newManager->flush();
        $newManager->refresh($data);
